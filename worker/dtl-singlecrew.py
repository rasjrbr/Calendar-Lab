...
DOMESTIC_LEG_RE = re.compile(r"^LA\s*\d{3,4}\s+[A-Z]{3}\s*-\s*[A-Z]{3}$", re.IGNORECASE)
...
def legs_bucket(legs_count: int) -> str:
    """Map integer leg count to table column key."""
    if legs_count <= 2:
        return "1-2"
    if legs_count <= 4:
        return "3-4"
...
def detect_leg(title: str) -> tuple[bool, bool, bool]:
    """
    Detect if title is a valid leg and classify special categories.
    ...
    clean = title.strip()
    if not DOMESTIC_LEG_RE.match(clean):
        return False, False, False
